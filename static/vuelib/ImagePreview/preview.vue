...
<template>
    <el-dialog
        ...
        <div slot="footer" class="preview-footer">
            <ul class="toolbar">
                <li class="icon">
                    <a @click="rotateImg(-1)"><img src="../../../src/assets/left_btn.png"></a>
                    <a @click="rotateImg(1)"><img src="../../../src/assets/right_btn.png"></a>
                </li>
                <li class="icon">
                    <a @click="zoom(1)"><i class="el-icon-circle-plus-outline"></i></a>
                    <a @click="zoom(-1)"><i class="el-icon-remove-outline"></i></a>
                </li>
                <li>
...
export default {
    ...
    data () {
        return {
            ...
            width: 960, // 弹窗高宽
            previewWidth: 0, // 容器高宽
            previewHeight: 480, // 当前显示图片地址
            currentSrc: '', // 当前图片位置
            loading: true, // 图片加载时loading层
            loadError: false, // 加载失败
            rotate: 0, // 旋转角度
            imageObj: null, // 图片对象
            imgWidth: 'auto', // 图片高宽
            imgHeight: 'auto',
            imgMarginLeft: 0, // 图片外填充
            imgMarginTop: 0,
            zoomRatio: 0.2, // 缩放比率：20%
            zoomWidth: 0, // 每次缩放量
            zoomHeight: 0,
            zoomTime: 0, // 缩放次数
            maxZoomTime: 20 // 最大缩放次数
        }
    },
    computed: {
        widths () {
    ...
    methods: {
        ...
        imgLoad (e) {
            ...
            let pW = previewBody.offsetWidth
            let pH = previewBody.offsetHeight
        ...
        rotateImg (ori) {
            ...
            let previewW = previewBody.offsetWidth
            let previewH = t.previewHeight
        ...
        zoom (zoom) {
            ...
            if (Math.abs(time) <= t.maxZoomTime) {
                ...
                let height = t.imageObj.height

                let zoomW = width + t.zoomWidth * zoom
                let zoomH = height + t.zoomHeight * zoom
