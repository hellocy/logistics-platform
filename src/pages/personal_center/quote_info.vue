<template>
    <div :class="prfClass">
        <div class="top">
            <div class="left"><i class="icon"></i>报价&分区信息</div>
        </div>
        <div class="wrap">
            <div class="left" @click="quoteFn">
                <img class="logo" src="../../assets/icon_1.png" />
                <span class="title">报价&分区信息</span>
            </div>
            <div class="right">
                <ul>
                    <li>
                        <div class="list_submit" @click="statue('submit')">
                            ...
                            <span class="title">{{ quoteInfo.wait_submit }}</span>
                        </div>
                        <div class="list_reject" @click="statue('reject')">
                            <i class="icon"></i>
                            <a class="btn" href="javascript:;">审核驳回</a>
                            <span class="title">{{ quoteInfo.review_reject }}</span>
                        </div>
                    </li>
                    <li>
                        <div class="list_audit" @click="statue('in_review')">
                            ...
                            <span class="title">{{ quoteInfo.in_review }}</span>
                        </div>
                    </li>
                    <li>
                        <div class="list_success" @click="statue('pass')">
                            <i class="icon"></i>
                            <a class="btn" href="javascript:;">审核完成</a>
                            <span class="title">{{ quoteInfo.pass_audit }}</span>
                        </div>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    methods: {
        ...
        statue (type) {
            switch (type) {
            case 'submit':
                this.$store.dispatch('common/priceStatue', { price_statue: 1 }) // 待提交
                break
            case 'in_review':
                this.$store.dispatch('common/priceStatue', { price_statue: 2 }) // 审核中
                break
            case 'pass':
                this.$store.dispatch('common/priceStatue', { price_statue: 3 }) // 审核通过
                break
            case 'reject':
                this.$store.dispatch('common/priceStatue', { price_statue: 4 }) // 审核驳回
                break
            default:
                break
            }
